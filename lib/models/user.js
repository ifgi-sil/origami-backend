...
const { model: Group } = require('./group');
...
userSchema.set('toJSON', {
  ...
  transform: function transform (doc, ret) {
    const { userName, firstName, lastName, email, games, friends } = ret;

    return { userName, firstName, lastName, email, games, friends };
  }
});
...
userSchema.methods.createGroup = function createGroup (params) {
  const user = this;

  return Group
    .initNew(params)
    .then(function (newGroup) {
      newGroup.owner.addToSet(user._id);
    });
};
