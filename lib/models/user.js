...
userSchema.set('toJSON', {
  ...
  transform: function transform (doc, ret) {
    const { _id, userName, firstName, lastName, email, games, invitedGames } = ret;

    return { _id, userName, firstName, lastName, email, games, invitedGames };
  }
});
...
userSchema.methods.checkGameOwner = function (gameId) {
  const user = this;

  // first check if the game belongs to this user
  if (!user.games) {
    throw new Error('User does not own this game', { type: 'ForbiddenError' });
  }

  const userOwnsGame = user.games.some(b => b.equals(gameId));

  if (userOwnsGame === false) {
    throw new Error('User does not own this game', { type: 'ForbiddenError' });
  }
...
userSchema.methods.checkPlayerGamePrivilage = function (gameId) {
  const user = this;

  // first check if the game belongs to this user
  if (!user.invitedGames) {
    throw new Error('User is not invited to any games', { type: 'ForbiddenError' });
  }

  const userOwnsGame = user.invitedGames.some(b => b.equals(gameId));

  if (userOwnsGame === false) {
    throw new Error('User is not invited to game', { type: 'ForbiddenError' });
  }

  return true;
};

userSchema.methods.getGames = function getGames () {
  return Game
    .find({ _id: { $in: this.games } })
    .then(function (games) {
      return games;
    });
};

userSchema.methods.getPrivateGame = function getPrivateGame (gameId) {
  return Game
    .findOne({ _id: gameId })
    .then(function (game) {
      return game;
...
userSchema.methods.getInvitedGames = function getInvitedGames () {
  return Game
    .find({ _id: { $in: this.invitedGames } })
    .then(function (games) {
      return games;
    });
};
...
userSchema.statics.initPasswordReset = function initPasswordReset ({ email }) {
  return this.findOne({ email: email.toLowerCase() })
    ...
    .then(function (user) {
      if (!user) {
        throw new Error('Password reset for this user not possible', { type: 'ForbiddenError' });
      }
  ...
  return this.findOne({ resetPasswordToken: token })
    ...
    .then(function (user) {
      ...
      if (moment.utc().isAfter(moment.utc(user.resetPasswordExpires))) {
        throw new Error('Password reset token expired', { type: 'ForbiddenError' });
      }
...
userSchema.statics.addInvitedGame = function ({ email, gameId }) {
  return this.findOne({ email: email.toLowerCase() })
    .exec()
    .then(function (user) {
      if (!user) {
        throw new Error('User not found!', { type: 'ForbiddenError' });
      }

      user.invitedGames.addToSet(gameId);
